...
void explodeNetch(std::string netch, Proxy &node)
{
    ...
    case "Trojan"_hash:
        ...
        if (group.empty())
            group = TROJAN_DEFAULT_GROUP;
        trojanConstruct(node, group, remark, address, port, password, transprot, host, path, fp, sni, tls == "true", udp, tfo, scv);
        break;
    case "Snell"_hash:
        obfs = GetMember(json, "OBFS");
        host = GetMember(json, "Host");
    ...
    for (auto &x : proxies)
    {
        ...
        */
        ...
        case "trojan"_hash: // surge 4 style trojan proxy
            ...
            trojanConstruct(node, TROJAN_DEFAULT_GROUP, remarks, server, port, password, "", host, "", fp, sni, true, udp, tfo, scv);
            break;
        case "snell"_hash:
            server = trim(configs[1]);
            port = trim(configs[2]);
        ...
        default:
            ...
            case "trojan"_hash: // quantumult x style trojan link
                ...
                trojanConstruct(node, TROJAN_DEFAULT_GROUP, remarks, server, port, password, "", host, "", fp, sni, tls == "true", udp, tfo, scv, tls13);
                break;
